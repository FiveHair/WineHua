...
assemble_pad() {
    ...
    # Keep runtime data inside the unified rawfile archive. The Wine audio
    # driver resolves MIDI_SOUNDFONT_PATH relative to the extracted runtime.
    ...
    if [ -d "$BUILD_DIR/wine-i386-pe" ]; then
        ...
        # 32-bit executable stubs required by WoW64 prefix setup and normal
        # process launching. In particular wineboot invokes the 32-bit
        # rundll32 for the Wow64Install section; omitting it leaves 32-bit COM
        # classes (DirectInput/DirectSound/MMDevice, etc.) unregistered.
        for exe in "$BUILD_DIR/wine-i386-pe/programs/notepad/i386-windows/notepad.exe" \
                   ...
                   "$BUILD_DIR/wine-i386-pe/programs/rundll32/i386-windows/rundll32.exe" \
                   "$BUILD_DIR/wine-i386-pe/programs/regsvr32/i386-windows/regsvr32.exe" \
                   "$BUILD_DIR/wine-i386-pe/programs/start/i386-windows/start.exe"; do
