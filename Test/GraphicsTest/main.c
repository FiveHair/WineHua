/*
 * WineHua standalone graphics test.
 */
...
static BOOL validate_graphics_backend(const struct app_state *state, char *reason, size_t reason_size)
{
    ...
    if (!state->running_under_wine)
    {
        snprintf(reason, reason_size,
                 "Native Windows runtime detected. Skipping Wine/VirGL environment checks and running visual demo only.");
        return TRUE;
    }
    ...
    if (!gfx->active_backend || lstrcmpiA(gfx->active_backend, "virgl"))
    {
        snprintf(reason, reason_size,
                 ...
                 winehua_graphics_env_or(gfx->requested_backend, "?"),
                 winehua_graphics_env_or(gfx->active_backend, "?"),
                 winehua_graphics_env_or(gfx->virgl_ready, "?"),
                 winehua_graphics_env_or(gfx->guest_gfx_ready, "?"),
                 winehua_graphics_env_or(gfx->guest_gfx_mode, "?"),
                 winehua_graphics_env_or(gfx->virgl_socket_ready, "?"),
                 winehua_graphics_env_or(gfx->virgl_library_ready, "?"),
                 winehua_graphics_env_or(gfx->frame_transport, "?"),
                 winehua_graphics_env_or(gfx->graphics_note, "(none)"));
        return gfx->force_gl;
    }
...
{
    static const WCHAR class_name[] = L"WineHuaGraphicsTestWindow";
    WNDCLASSEXW wc = {0};
    ...
    {
        fprintf(stderr, "winehua_graphics_test: RegisterClassExW failed: %lu\n", GetLastError());
        return FALSE;
    }

    AdjustWindowRect(&rect, style, FALSE);
    state->hwnd = CreateWindowExW(0, class_name, L"WineHua Graphics Test",
                                  style, CW_USEDEFAULT, CW_USEDEFAULT,
                                  rect.right - rect.left, rect.bottom - rect.top,
                                  NULL, NULL, wc.hInstance, state);
    if (!state->hwnd)
    {
        fprintf(stderr, "winehua_graphics_test: CreateWindowExW failed: %lu\n", GetLastError());
        return FALSE;
    }
    ...
    if (!state->hdc)
    {
        fprintf(stderr, "winehua_graphics_test: GetDC failed: %lu\n", GetLastError());
        return FALSE;
    }
...
static BOOL init_opengl(struct app_state *state)
{
    ...
    if (!format)
    {
        fprintf(stderr, "winehua_graphics_test: ChoosePixelFormat failed: %lu\n", GetLastError());
        return FALSE;
    }

    if (!SetPixelFormat(state->hdc, format, &pfd))
    {
        fprintf(stderr, "winehua_graphics_test: SetPixelFormat failed: %lu\n", GetLastError());
        return FALSE;
    }
    ...
    if (!state->glrc)
    {
        fprintf(stderr, "winehua_graphics_test: wglCreateContext failed: %lu\n", GetLastError());
        return FALSE;
    }

    if (!wglMakeCurrent(state->hdc, state->glrc))
    {
        fprintf(stderr, "winehua_graphics_test: wglMakeCurrent failed: %lu\n", GetLastError());
        return FALSE;
    }
    ...
    fprintf(stderr, "winehua_graphics_test: GL vendor=%s renderer=%s version=%s\n",
            (const char *)glGetString(GL_VENDOR),
            (const char *)glGetString(GL_RENDERER),
            (const char *)glGetString(GL_VERSION));
    fprintf(stderr, "winehua_graphics_test: env requested=%s active=%s virgl_ready=%s guest_gfx_ready=%s guest_gfx_mode=%s socket_ready=%s library_ready=%s transport=%s zero_copy=%s shm_fallback=%s note=%s\n",
            ...
            winehua_graphics_env_or(state->gfx_env.frame_zero_copy, "(null)"),
            winehua_graphics_env_or(state->gfx_env.shm_fallback, "(null)"),
...
static void setup_perspective(const struct app_state *state)
{
    ...
    glLoadIdentity();
    glFrustum(-right, right, -top, top, z_near, 40.0);
}
...
static void draw_seven_segment_digit(float x, float y, float scale, char ch)
{
    ...
    if (ch >= '0' && ch <= '9') mask = digit_segments[ch - '0'];
    else if (ch == '-') mask = SEG_G;
...
static void parse_args(struct app_state *state, int argc, char **argv)
{
    ...
    for (i = 1; i < argc; ++i)
    {
        if (!lstrcmpiA(argv[i], "--loop")) state->loop_forever = TRUE;
...
int main(int argc, char **argv)
{
    ...
    {
        fprintf(stderr, "winehua_graphics_test: QueryPerformanceFrequency failed\n");
        return 1;
    }

    if (!validate_graphics_backend(&state, reason, sizeof(reason)))
    {
        fprintf(stderr, "winehua_graphics_test: %s\n", reason);
        MessageBoxA(NULL, reason, "WineHua Graphics Test", MB_OK | MB_ICONWARNING);
        return 4;
    }
    if (reason[0])
    {
    ...
    if (!init_opengl(&state))
    {
        MessageBoxA(state.hwnd,
                    "OpenGL initialization failed. Check ChoosePixelFormat / WGL / virgl readiness.",
    ...
    while (state.running)
    {
        ...
        render_frame(&state,
                     (float)(((double)(now.QuadPart - start.QuadPart) / (double)state.freq.QuadPart) * 72.0),
                     (float)(((double)(now.QuadPart - start.QuadPart) / (double)state.freq.QuadPart) * 1.8));
    ...
    fprintf(stderr, "winehua_graphics_test: finished after %u frames\n", frames);
